...
void ofApp::setup(){
    ...
    // 画像(ofImage)に変換
    ofxCv::toOf(saliencyMap_color.clone(), outputOfImg3);
    outputOfImg3.update();
...
//    outputOfImg.draw(ofGetWidth()/2,0,ofGetWidth()/2, ofGetHeight()/2);
    // 顕著性マップを出力
    outputOfImg2.draw(0,ofGetHeight()/2,ofGetWidth()/2, ofGetHeight()/2);
    // 顕著性マップのヒートマップを出力
    outputOfImg3.draw(ofGetWidth()/2,ofGetHeight()/2,ofGetWidth()/2, ofGetHeight()/2);

    // Label
    ofDrawBitmapStringHighlight("original", 20, 20);
//    ofDrawBitmapStringHighlight("saliencyMap", ofGetWidth()/2+20, 20);
    ofDrawBitmapStringHighlight("saliencyMap", 20, ofGetHeight()/2+20);
